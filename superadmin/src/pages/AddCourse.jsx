...
export default function AddCourse({ Layout = DefaultLayout }) {
    ...
    const fetchCourseData = async () => {
        try {
            setLoading(true);
            setError('');
            const res = await api.get(`/superadmin/courses/${courseId}`);
            const course = res.data;

            setFormData({
                organizerId: course.organizer || '',
                title: course.title || '',
    ...
    const steps = [
        { id: 1, title: 'Organizer', icon: Users, desc: 'Select Organizer' },
        { id: 2, title: 'Essentials', icon: FileText, desc: 'Basic info & Logistics' },
        { id: 3, title: 'Visuals', icon: ImageIcon, desc: 'Thumbnails & Banners' },
        { id: 4, title: 'Content', icon: MonitorPlay, desc: 'Details & Credits' },
        { id: 5, title: 'Experts', icon: Users, desc: 'Speakers & Hosts' },
    ];
    ...
    const handleSaveCourse = async (status = 'active') => {
        try {
            ...
            data.append('organizer', formData.organizerId);
            ...
            if (courseId) {
                await api.put(`/superadmin/courses/${courseId}`, data);
            } else {
                await api.post('/superadmin/courses', data);
            }
                                    ...
                                    {currentStep === 1 && (
                                        <div className="space-y-6">
                                            <div className="flex items-center justify-between">
                                                <h2 className="text-2xl font-bold text-[#25225B]">Select Organizer</h2>
                                                <div className="relative w-64">
                                                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                                                    <input 
                                                        type="text" 
                                                        placeholder="Search organizers..." 
                                                        className="w-full pl-10 pr-4 py-2 bg-slate-50 rounded-xl border-none outline-none focus:ring-2 focus:ring-[#25225B]/20"
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[500px] overflow-y-auto pr-2">
                                                {organizers
                                                    .filter(org => 
                                                        org.organizationName?.toLowerCase().includes(organizerSearch.toLowerCase()) || 
                                                        org.email?.toLowerCase().includes(organizerSearch.toLowerCase())
                                                    )
                                                    .map(org => (
                                                    <div 
                                                        key={org._id} 
                                                        onClick={() => setFormData({...formData, organizerId: org._id})}
                                                        className={`p-4 border-2 rounded-xl cursor-pointer transition-all flex items-center gap-4 ${formData.organizerId === org._id ? 'border-[#25225B] bg-[#25225B]/5 shadow-lg shadow-[#25225B]/5' : 'border-slate-100 hover:border-[#25225B]/50 hover:bg-slate-50'}`}
                                                    >
                                                        <div className={`w-12 h-12 rounded-full flex items-center justify-center text-lg font-bold ${formData.organizerId === org._id ? 'bg-[#25225B] text-white' : 'bg-slate-100 text-slate-500'}`}>
                                                            {org.organizationName?.charAt(0) || '?'}
                                                        </div>
                                                        <div>
                                                            <div className="font-bold text-[#25225B]">{org.organizationName}</div>
                                                            <div className="text-sm text-slate-500">{org.email}</div>
                                                        </div>
                                                        {formData.organizerId === org._id && (
                                                            <div className="ml-auto text-[#25225B]">
                                                                <CheckCircle size={24} />
                                                            </div>
                                                        )}
                                                    </div>
                ...
                <div className={`fixed bottom-0 left-80 right-0 z-50 p-8 bg-gradient-to-t pointer-events-none ${isDark
                    ...
                    <div className="max-w-[1600px] mx-auto pointer-events-auto">
                        <div className={`max-w-[66.666%] flex gap-4 p-4 backdrop-blur-xl border shadow-2xl rounded-2xl ${isDark
                            ...
                            <button
                                onClick={() => {
                                    if (currentStep === 1 && !formData.organizerId) {
                            ...
                            >
                                {saving ? (
                                    ...
                                    <>
                                        {currentStep === 5 ? 'Publish Course' : 'Continue'}
                                        <ArrowRight size={20} />
                                    </>
                                )}
                            </button>
